    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.status === 'success') {
                if (!editingItem) {
                    setFormData({ 開始日期: '', 結束日期: '', 地區: '', 幣別: 'TWD', 每日金額: '', 金額: '', 備註: '' });
                }
                onSubmitSuccess();
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                    ...
                    <input type="date" value={formData.開始日期} onChange={e => setFormData({ ...formData, 開始日期: e.target.value })} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg" required />
                </div>
                <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('end_date')}</label>
                    <input type="date" value={formData.結束日期} onChange={e => setFormData({ ...formData, 結束日期: e.target.value })} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg" required />
                </div>
                <div className="space-y-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
                ...
                <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('daily_amount')}</label>
                    <input type="number" value={formData.每日金額} onChange={e => setFormData({ ...formData, 每日金額: e.target.value })} className="w-full p-2.5 bg-white border border-gray-200 rounded-lg" required />
                </div>
                <div className="space-y-1">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('total_amount')}</label>
